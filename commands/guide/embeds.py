...
def progress_bar(i: int, n: int, width: int = 10) -> str:
    """▰▰▰▱▱▱ style bar — i is the 0-based current step of n."""
    filled = max(1, round(width * (i + 1) / n)) if n else 0
...
def _turn_card(raid: str, title: str, table: dict, row_index: int,
    ...
    bar = f"{progress_bar(row_index, len(rows))}  **Step {row_index + 1} / {len(rows)}**"
    e = base_embed(raid, heading, bar + (f"\n{description}" if description else ""))
...
def sections_embed(raid: str) -> discord.Embed:
    ...
    return base_embed(raid, f"📖 {raid}",
                      ...
                      f"{r.get('players','?')} players • **Pick your role** "
                      "(or a section):")
